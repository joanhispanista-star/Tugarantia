...
do $prueba$
declare
  jefe    text := '3009990001';
  peon    text := '3009990002';
  raro    text := '3009990003';
  ido     text := '3009990004';
  alcance text[];
  pasos   text := '';
begin
  ...
  if exists (select 1 from public.equipo where celular in (jefe, peon, raro, ido)) then
    raise exception
  ...
  values (jefe, 'Prueba jefe',  'gerente', 'activo',   null),
         (peon, 'Prueba peon',  'asesor',  'activo',   jefe),
         (ido,  'Prueba ido',   'asesor',  'retirado', jefe);

  ---- 1. El gerente alcanza a los suyos ACTIVOS, y no al retirado.
  alcance := public.mi_alcance(jefe);
  if not (peon = any(alcance)) then
    raise exception 'FALLO 1: el gerente no alcanza a su asesor: %', alcance;
  end if;
  if ido = any(alcance) then
  ...
  alcance := public.mi_alcance(peon);
  if array_length(alcance, 1) <> 1 or alcance[1] <> peon then
  ...
  ---- CHECK de la tabla no deja meter un rol raro, asi que se prueba el caso
  ---- que SI se puede montar y que es el mismo camino: un celular desconocido.
  alcance := public.mi_alcance(raro);
  if coalesce(array_length(alcance, 1), 0) <> 0 then
  ...
  values ('PRUEBA-ASG-1', 'PRUEBA-ALC-1', peon, current_date, now())
  on conflict (id) do nothing;
  begin
    insert into public.asignaciones (id, persona_id, asesor, desde, actualizado)
    values ('PRUEBA-ASG-2', 'PRUEBA-ALC-1', jefe, current_date, now());
  ...
  delete from public.equipo      where celular in (jefe, peon, ido);
